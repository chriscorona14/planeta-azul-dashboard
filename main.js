...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const bindPdfExport = (btn) => {
        ...
        btn.addEventListener('click', async () => {
            ...
            const addPageToPDF = async (forcedSubtitle = "") => {
                ...
                // Ensure the capture window accommodates full width but expands if content is larger
                const actualWidth = contentToRender.scrollWidth;
                const desktopWidth = actualWidth > 1440 ? actualWidth : 1440;
                ...
                const canvas = await html2canvas(contentToRender, {
                    ...
                    windowHeight: contentToRender.scrollHeight + 100,
                    height: contentToRender.scrollHeight + 100,
                    onclone: (clonedDoc) => {
    ...
    const showPptxPreviewModal = (previewSlides, pptx, dateStr, btn, originalText) => {
        ...
        header.style.backgroundColor = '#1e293b'; // slate-800
        ...
        titleInfo.innerHTML = `<h2 style="margin: 0; font-family: 'Inter', sans-serif; font-size: 1.25rem;">Vista Previa de Exportación PPTX</h2>
                               <p style="margin: 4px 0 0; font-size: 0.875rem; color: #94a3b8;">Revisa el formato de los slides. Se reservó espacio en los de Estado de Resultados.</p>`;
        
        const actionControls = document.createElement('div');
        actionControls.style.display = 'flex';
        actionControls.style.gap = '12px';

        const cancelBtn = document.createElement('button');
        cancelBtn.textContent = 'Cancelar';
        cancelBtn.style.padding = '8px 16px';
        cancelBtn.style.backgroundColor = '#475569';
        ...
        downloadBtn.innerHTML = '<i data-lucide="download" style="width:16px; height:16px; display:inline-block; vertical-align:middle; margin-right:6px;"></i>Confirmar y Descargar';
        downloadBtn.style.padding = '8px 16px';
        downloadBtn.style.backgroundColor = '#0ea5e9'; // sky-500
        downloadBtn.style.color = 'white';
        downloadBtn.style.border = 'none';
        downloadBtn.style.borderRadius = '6px';
        downloadBtn.style.cursor = 'pointer';
        downloadBtn.style.fontFamily = "'Inter', sans-serif";
        downloadBtn.style.fontWeight = '500';

        actionControls.appendChild(cancelBtn);
        actionControls.appendChild(downloadBtn);
        ...
        previewSlides.forEach((slide, index) => {
            ...
            slideTitle.textContent = `Slide ${index + 2}: ${slide.title || 'Contenido'}`;
            slideTitle.style.color = '#cbd5e1';
            slideTitle.style.fontSize = '0.875rem';
            slideTitle.style.fontFamily = "'Inter', sans-serif";
            
            const slideAspect = document.createElement('div');
            slideAspect.style.position = 'relative';
            slideAspect.style.width = '100%';
            slideAspect.style.paddingTop = '56.25%'; // 16:9
            slideAspect.style.backgroundColor = '#ffffff';
            slideAspect.style.borderRadius = '4px';
            slideAspect.style.boxShadow = '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)';
            slideAspect.style.overflow = 'hidden';

            // Background mock (just solid white to mimic main bg)
            
            // Image
            const imgEl = document.createElement('img');
            imgEl.src = slide.imgData;
            imgEl.style.position = 'absolute';
            imgEl.style.left = `${(slide.imgX / slide.slideW) * 100}%`;
            imgEl.style.top = `${(slide.imgY / slide.slideH) * 100}%`;
            imgEl.style.width = `${(slide.imgW / slide.slideW) * 100}%`;
            imgEl.style.height = `${(slide.imgH / slide.slideH) * 100}%`;
            // Add a slight outline for clarity
            imgEl.style.boxShadow = '0 0 0 1px rgba(0,0,0,0.05)';
            
            slideAspect.appendChild(imgEl);

            if (slide.isPnL) {
                // approximate availW and dimensions
                const availW = slide.slideW - 3.0; // using the same logic
                
                const commBox = document.createElement('div');
                commBox.style.position = 'absolute';
                commBox.style.left = `${((availW + 0.4) / slide.slideW) * 100}%`;
                commBox.style.top = `${(0.2 / slide.slideH) * 100}%`;
                commBox.style.width = `${(2.4 / slide.slideW) * 100}%`;
                commBox.style.height = `${((slide.slideH - 0.4) / slide.slideH) * 100}%`;
                commBox.style.border = '1px solid #E5E7EB';
                commBox.style.backgroundColor = '#FFFFFF';
                commBox.style.padding = '8px';
                
                const commText = document.createElement('span');
                commText.textContent = 'Comentarios / Notas:';
                commText.style.color = '#005b96';
                commText.style.fontWeight = 'bold';
                commText.style.fontSize = '8px';
                commText.style.fontFamily = "'Segoe UI', sans-serif";
                
                commBox.appendChild(commText);
                slideAspect.appendChild(commBox);
            }
            
            wrap.appendChild(slideTitle);
            wrap.appendChild(slideAspect);
        ...
        downloadBtn.addEventListener('click', async () => {
            ...
            downloadBtn.innerHTML = '<i data-lucide="loader" class="spin-icon" style="width:16px; height:16px; display:inline-block; vertical-align:middle; margin-right:6px;"></i>Generando...';
            if (typeof lucide !== 'undefined') lucide.createIcons();
            
            try {
                await pptx.writeFile({ fileName: `Reportes_Ejecutivos_Maestros_${dateStr}.pptx` });
            } catch (err) {
                console.error(err);
                alert("Ocurrió un error al guardar el PPTX.");
            }
            closeAndRestore();
        });
    };

    const bindPptxExport = (btn) => {
        if (!btn) return;
        btn.addEventListener('click', async () => {
            btn.disabled = true;
            const originalText = btn.innerHTML;
            ...
            const addPageToPPTX = async (forcedSubtitle = "", isPnL = false) => {
                const layoutWrapper = document.querySelector('.layout-wrapper');
                const contentToRender = document.querySelector('.main-container');
                if (!contentToRender) return;
                ...
                // Pure aspect ratio calculation to prevent "estirado" (stretching) effect
                const slideW = 10;
                const slideH = 5.625;
                const imgRatio = canvas.width / canvas.height;
                
                let availW = slideW - 0.4;
                let availH = slideH - 0.4;
                
                // Reserve space for comments on PnL slide
                if (isPnL) {
                    availW = slideW - 3.0; // Reserve 2.8 inches + 0.2 margin
                }
                ...
                let imgX = (slideW - imgW) / 2;
                if (isPnL) {
                    // Center within the available left space
                    imgX = (availW - imgW) / 2 + 0.2; 
                }
                ...
                if (isPnL) {
                    // Draw comments box
                    const rectType = (typeof pptx.ShapeType !== 'undefined' && pptx.ShapeType.rect) ? pptx.ShapeType.rect : 'rect';
                    slide.addShape(rectType, { 
                        x: availW + 0.4, y: 0.2, w: 2.4, h: slideH - 0.4, 
                        fill: 'FFFFFF', line: { color: 'E5E7EB', width: 1 }, rectRadius: 0.1 
                    });
                    
                    slide.addText('Comentarios / Notas:', { 
                        shape: rectType, 
                        x: availW + 0.4, y: 0.2, w: 2.4, h: 0.4, 
                        fontSize: 12, bold: true, color: '005b96', fontFace: 'Segoe UI', valign: 'top', align: 'left', margin: 10
                    });
                }
                
                previewSlides.push({
                    title: forcedSubtitle,
                    imgData: imgData,
                    imgW: imgW,
                    imgH: imgH,
                    imgX: imgX,
            ...
            const showViewAndSync = (viewId, menuId) => {
                ...
                if (document.getElementById('view-kpi')) {
                    showViewAndSync('view-kpi', 'menu-kpi'); 
                    await sleep(800); 
                    await addPageToPPTX("Dashboard | " + (previousYTD ? "YTD" : "Mensual"));
                }
                ...
                if (document.getElementById('view-resumen')) {
                    showViewAndSync('view-resumen', 'menu-resumen'); 
                    await sleep(800); 
                    await addPageToPPTX("Gestión Corporativa | " + (previousYTD ? "YTD" : "Mensual"));
                }
                ...
                if (document.getElementById('view-resumen-comercial')) {
                    ...
                    toggleYTD(false); clickElement('btn-comercial-resumen'); await sleep(800); await addPageToPPTX("Mensual | Resumen de Ventas");
                    toggleYTD(true); clickElement('btn-comercial-resumen'); await sleep(800); await addPageToPPTX("YTD | Resumen de Ventas");
                    
                    toggleYTD(false); clickElement('btn-comercial-variacion'); await sleep(800); await addPageToPPTX("Mensual | Análisis de Variación (vs PPTO / vs AA)");
                    toggleYTD(true); clickElement('btn-comercial-variacion'); await sleep(800); await addPageToPPTX("YTD | Análisis de Variación (vs PPTO / vs AA)");
                    
                    toggleYTD(false); clickElement('btn-comercial-mom'); await sleep(800); await addPageToPPTX("Mensual | Variación MoM");
                }

                // 4. Estado de Resultado (Mensual y YTD)
                if (document.getElementById('view-preliminar')) {
                    showViewAndSync('view-preliminar', 'menu-preliminar');
                    toggleYTD(false); await sleep(800); await addPageToPPTX("Mensual", true);
                    toggleYTD(true); await sleep(800); await addPageToPPTX("YTD", true);
                }
                ...
                if (document.getElementById('view-balance')) {
                    ...
                    clickElement('btn-balance-resumen');
                    await sleep(800);
                    await addPageToPPTX("Balance Sheet | Resumen | " + (previousYTD ? "YTD" : "Mensual"));
                }
                ...
                if (document.getElementById('view-cashflow')) {
                    ...
                    clickElement('btn-cashflow-resumen');
                    await sleep(800);
                    await addPageToPPTX("Cash Flow | Resumen | " + (previousYTD ? "YTD" : "Mensual"));
                }
                ...
                if (document.getElementById('view-cxp')) {
                    ...
                    clickElement('btn-cxp-resumen');
                    await sleep(800);
                    await addPageToPPTX("Resumen de Cuentas por Pagar (DOP)");
                }
                ...
                if (document.getElementById('view-deuda')) {
                    showViewAndSync('view-deuda', 'menu-deuda');
                    await sleep(800);
                    await addPageToPPTX("Zoom in Deuda");
                }
                ...
                if (document.getElementById('view-ventas-ceo')) {
                    showViewAndSync('view-ventas-ceo', 'menu-ventas-ceo');
                    toggleYTD(previousYTD);
                    clickElement('btn-ventas-vol'); await sleep(800); await addPageToPPTX("Métrica: Volumen (k de Unidades)");
                    clickElement('btn-ventas-monto'); await sleep(800); await addPageToPPTX("Métrica: Monto (mDOP)");
                    clickElement('btn-ventas-precio'); await sleep(800); await addPageToPPTX("Métrica: Precio Unitario");
                }
                ...
                if (document.getElementById('view-costo-unitario') && typeof window.hasCostoUnitarioData === 'function' && window.hasCostoUnitarioData()) {
                    ...
                    clickElement('btn-costo-vista-resumen');
                    await sleep(800);
                    await addPageToPPTX("Costo Unitario | Botellón | Resumen");
                    
                    clickElement('btn-costo-botella');
                    clickElement('btn-costo-vista-resumen');
                    await sleep(800);
                    await addPageToPPTX("Costo Unitario | Botella 0.5 LTS | Resumen");
                }

                const dateStr = new Date().toISOString().slice(0, 10);
                
                // Keep the menuid sync in check for restoration BEFORE showing modal
                toggleYTD(previousYTD);
                const navKeyMap = {
                    'view-kpi': 'menu-kpi', 'view-resumen': 'menu-resumen', 'view-preliminar': 'menu-preliminar',
                    'view-resumen-comercial': 'menu-resumen-comercial', 'view-ventas-ceo': 'menu-ventas-ceo',
                ...
                // Show modal
                showPptxPreviewModal(previewSlides, pptx, dateStr, btn, originalText);

            } catch(e) {
                console.error("Error generating Master PPTX:", e);
                alert("Ocurrió un error al generar el PPTX: " + String(e.message || e));
                btn.disabled = false;
                btn.innerHTML = originalText;
                if (typeof lucide !== 'undefined') lucide.createIcons();
            }
